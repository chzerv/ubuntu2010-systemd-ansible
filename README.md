# Ubuntu 20.10 (Groovy) Container Image for Ansible Testing

This Dockerfile builds a Ubuntu 20.10 based container, capable to use `systemd`, mainly for Ansible role testing.
...
3. Run a container:
   ...
   ```shell
   docker run -d --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro ubuntu2010-systemd-ansible:latest
   ```
